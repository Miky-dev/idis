"""
tools_handmouse.py — Wrapper IDIS per hand_mouse_controller.py
Avvia/ferma il controller a comando vocale in un thread daemon.
"""
...
# ── Loop che esegue il codice originale ───────────────────
def _loop():
    global _attivo
    import cv2
    ...
    # ── Stessi parametri del file originale ──
    ...
    # ── Utility (copiate 1:1 dall'originale) ──
    def dist(a, b):
    ...
    # ── Loop principale (identico all'originale, aggiunto _stop_event) ──
...
def attiva_controllo_mano() -> str:
    ...
    """
    global _attivo, _thread
    if _attivo:
        return "Il controllo mano e gia attivo. Guarda la finestra HandMouse."
    try:
        import cv2, mediapipe, pyautogui
    except ImportError as e:
        return f"Dipendenze mancanti: {e}. Esegui: pip install opencv-python mediapipe pyautogui"

    _stop_event.clear()
    _attivo = True
    _thread = threading.Thread(target=_loop, daemon=True, name="HandMouse")
    _thread.start()
    return (
        "Controllo mano attivato. Si apre la finestra webcam.\n"
